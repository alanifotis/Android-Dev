...
public class FmFavoriteActivity extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mExitListener = new OnExitListener() {
            @Override
            public void onExit() {
    ...
    private Cursor getData() {
        Cursor cursor = mContext.getContentResolver().query(Station.CONTENT_URI,
                FmStation.COLUMNS, null, null, Station.FREQUENCY);
        return cursor;
    }
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            if (mCursor != null && mCursor.moveToFirst()) {
                ...
                final int stationFreq = mCursor.getInt(mCursor
                        .getColumnIndex(Station.FREQUENCY));
                String name = mCursor.getString(mCursor
                        .getColumnIndex(Station.STATION_NAME));
                String rds = mCursor.getString(mCursor
                        .getColumnIndex(Station.RADIO_TEXT));
                final int isFavorite = mCursor.getInt(mCursor
                        .getColumnIndex(Station.IS_FAVORITE));

                if (null == name || "".equals(name)) {
                    name = mCursor.getString(mCursor
                            .getColumnIndex(Station.PROGRAM_SERVICE));
                }
